...
public:
  ...
  void print_adjacency_list() {
    for (const auto& pair : adjacency_list) {
        cout << "From Station " << pair.first << ":" << endl;
        for (const auto& edge : pair.second) {
            cout << "  To Station " << edge.first << " (Duration: " << edge.second << " minutes)" << endl;
       }
      cout << endl;
    }
  }
  ...
  void print_adjacency_list_matrix() {
    cout << "\t";
    for (const auto& pair : adjacency_list) {
        cout << pair.first << "\t";
    }
    ...
    for (const auto& pair : adjacency_list) {
        cout << pair.first << "\t"; 
        for (const auto& pair2 : adjacency_list) {
            bool connected = false;
            for (const auto& edge : pair.second) {
                if (edge.first == pair2.first) {
                    cout << edge.second << "\t";
                    connected = true;
                    break;
                }
  ...
  void print_buses() {
    ...
    for (const auto& bus : buses) {
        cout << "Bus ID: " << bus.first << ", Route: " << bus.second.first << " to " << bus.second.second << endl;
    }
  }
